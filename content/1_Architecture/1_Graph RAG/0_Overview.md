...
The architectures below are presented to support **specific use cases** (e.g. when you need global summaries, or a separate vector store). From a **project perspective**, it is usually better to **start with a simple graph RAG** (e.g. knowledge graph or unified graph with chunk + entity retrieval) and **add features incrementally**—for example community detection and hierarchical retrieval—while **monitoring impact on accuracy, latency, or other metrics**. That way you can validate that each addition brings real benefit before increasing complexity.
